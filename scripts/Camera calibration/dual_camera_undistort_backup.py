...
Shows left and right cameras side by side with right camera rotated 180 degrees.
"""
...
def main():
    ...
    while True:
        ...
        # Rotate right camera by 180 degrees
        right_und = cv2.rotate(right_und, cv2.ROTATE_180)
        ...
        if args.show_raw:
            ...
            right_combo = cv2.rotate(right_combo, cv2.ROTATE_180)  # Rotate the entire right combo
            ...
            put_text(dual_combo, f"Right: raw | undistorted (rotated 180°)  ({right_info})", y=H+30)
            ...
            put_text(dual_combo, f"Right: undistorted (rotated 180°)  ({right_info})", y=60)
            put_text(dual_combo, f"FPS ~ {fps_disp:.1f}", y=90)

        cv2.imshow("dual_cameras", dual_combo)
