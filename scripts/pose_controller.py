...
class PoseController(Node):
    def __init__(self):
        ...
        self.declare_parameter('yaw_tol', 0.05)   # desired yaw accuracy (rad)
        ...
        self.declare_parameter('Kp_angular_yaw', 2.0) # Default same as straight line #3.81
        self.declare_parameter('Ki_angular_yaw', 0.5) # Default same as straight line #1.35
        self.declare_parameter('Kd_angular_yaw', 0.0) # Default same as straight line #0.18
    ...
    def control_function(
        ...
        if self.waypoint_achieved and self.waypoint_navigation_active:
            ...
            self.get_logger().info(f'[WP_NAV] WAYPOINT ACHIEVED! Moving to next waypoint...')
    ...
    def waypoint_array_callback(self, msg):
        ...
        Format: [x1,y1,dc1, x2,y2,dc2, ...] or legacy [x1,y1, x2,y2, ...]
        or [0.0] for navigation start signal
        """
        ...
        if len(msg.data) % 3 == 0:
            ...
            self.get_logger().error(
                f"Invalid waypoint array: length {len(msg.data)} not divisible by 2 or 3"
            )
            return
    ...
    def set_next_waypoint_target(self):
        ...
        if self.previous_waypoint is not None:
            ...
            self.get_logger().info(f'[WP_NAV] Target yaw calculation: prev=({self.previous_waypoint[0]:.2f}, {self.previous_waypoint[1]:.2f}) -> '
                                  f'current=({self.target_x:.2f}, {self.target_y:.2f}) -> dx={dx:.2f}, dy={dy:.2f} -> '
